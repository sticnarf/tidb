...
func (s *testCommitterSuite) TestPrewriteRollback(c *C) {
	...
	c.Assert(err, IsNil)
	err = committer.prewriteMutations(NewBackoffer(ctx, PrewriteMaxBackoff), committer.keys)
	c.Assert(err, IsNil)
	...
	c.Assert(err, IsNil)
	err = committer.commitMutations(NewBackoffer(ctx, CommitMaxBackoff), [][]byte{[]byte("a")})
	c.Assert(err, IsNil)
...
func (s *testCommitterSuite) TestContextCancel(c *C) {
	...
	err = committer.prewriteMutations(backoffer, committer.keys)
...
func (s *testCommitterSuite) TestContextCancelRetryable(c *C) {
	...
	committer, err := newTwoPhaseCommitterWithInit(txn1, 0)
	c.Assert(err, IsNil)
	err = committer.prewriteMutations(NewBackoffer(context.Background(), PrewriteMaxBackoff), committer.keys)
	c.Assert(err, IsNil)
	// txn3 writes "c"
...
func (s *testCommitterSuite) TestCommitBeforePrewrite(c *C) {
	...
	c.Assert(err, IsNil)
	ctx := context.Background()
	err = committer.cleanupMutations(NewBackoffer(ctx, cleanupMaxBackoff), committer.keys)
	c.Assert(err, IsNil)
	err = committer.prewriteMutations(NewBackoffer(ctx, PrewriteMaxBackoff), committer.keys)
	c.Assert(err, NotNil)
...
func (s *testCommitterSuite) TestRejectCommitTS(c *C) {
	...
	c.Assert(err, IsNil)
	batch := batchMutations{region: loc.Region, keys: [][]byte{[]byte("x")}}
	...
	err = committer.commitMutations(bo, committer.keys)
	c.Assert(err, IsNil)
...
func (s *testCommitterSuite) getLockInfo(c *C, key []byte) *kvrpcpb.LockInfo {
	...
	loc, err := s.store.regionCache.LocateKey(bo, key)
	c.Assert(err, IsNil)
	batch := batchMutations{region: loc.Region, keys: [][]byte{key}}
	req := committer.buildPrewriteRequest(batch, 1)
	resp, err := s.store.SendReq(bo, req, loc.Region, readTimeoutShort)
	c.Assert(err, IsNil)
	c.Assert(resp.Resp, NotNil)
...
func (s *testCommitterSuite) TestPkNotFound(c *C) {
	...
	err = txn1.committer.pessimisticRollbackMutations(bo, [][]byte{k1})
	c.Assert(err, IsNil)
